...
def cdad_shc_just_func(data_right_binned, data_left_binned, min_plot_num_en, max_plot_num_en, av='on'):
    ...
    if min_plot_num_en == max_plot_num_en:
        data_rightsel = data_right_binned.sel(
            energy_corr=min_plot_num_en, method="nearest")
        data_leftsel = data_left_binned.sel(
            energy_corr=min_plot_num_en, method="nearest")
    else:
        data_rightsel = data_right_binned.sel(energy_corr=slice(
            min_plot_num_en, max_plot_num_en)).sum(dim='energy_corr')
        data_leftsel = data_left_binned.sel(energy_corr=slice(
            min_plot_num_en, max_plot_num_en)).sum(dim='energy_corr')
...
def cdad_plot(data_right, data_left, length_subplot_in, min_plot_num_en, max_plot_num_en,
    ...
    for i in range(length_subplot_in):
        ...
        if sing_val != 'none':
            data_rightsel = data_right_binned.sel(
                energy_corr=z, method="nearest")
            data_leftsel = data_left_binned.sel(
                energy_corr=z, method="nearest")
        else:
            data_rightsel = data_right_binned.sel(
                energy_corr=slice(z, z+z_factor)).sum(dim='energy_corr')
            data_leftsel = data_left_binned.sel(
                energy_corr=slice(z, z+z_factor)).sum(dim='energy_corr')
...
def cdad_dc_comparison(data_right, data_left, x_cut, y_cut, min_plot_num_updc,
    ...
    data_right.sel(energy_corr=slice(min_plot_num_updc, max_plot_num_updc)).sum(
        dim='energy_corr').plot()
    ...
    data_right.sel(energy_corr=slice(min_plot_num_dc, max_plot_num_dc)).sum(
        dim='energy_corr').plot()
    plt.xlabel('$ \Phi_x$')
    plt.ylabel('$ \Theta_y$')
    if min_plot_num_dc == max_plot_num_dc:
    ...
    data_right.sel(energy_corr=slice(min_plot_num_lowdc, max_plot_num_lowdc)).sum(
        dim='energy_corr').plot()
    plt.xlabel('$ \Phi_x$')
    plt.ylabel('$ \Theta_y$')
    if min_plot_num_lowdc == max_plot_num_lowdc:
...
def image_plot(data_binned, length_subplot_in, min_plot_num_en, max_plot_num_en,
    ...
    for i in range(length_subplot):
        ...
        if i % 2 == 0:
            ...
            if sing_val != 'none':
                datasel = data_binned.sel(energy_corr=slice(
    ...
    for i in range(length_subplot_in):
        ...
        if sing_val != 'none':
            ...
            datasel = data_binned.sel(energy_corr=slice(
                z, z+z_factor)).sum(dim='energy_corr')
...
def onelinecut(data, energy_slice_down, energy_slice_up, line_cut_x, line_cut_y, central_angle_x=0, central_angle_y=0):
    ...
    if energy_slice_down == energy_slice_up:
        selecteddata_x = data.sel(energy_corr=energy_slice_down, method="nearest").sel(
            y=line_cut_x, method="nearest").values
        selecteddata_y = data.sel(energy_corr=energy_slice_down, method="nearest").sel(
            x=line_cut_y, method="nearest").values
    else:
        selecteddata_x = data.sel(energy_corr=slice(energy_slice_down, energy_slice_up)).sum(
            dim='energy_corr').sel(y=line_cut_x, method="nearest").values
        selecteddata_y = data.sel(energy_corr=slice(energy_slice_down, energy_slice_up)).sum(
            dim='energy_corr').sel(x=line_cut_y, method="nearest").values
    ...
    if energy_slice_down == energy_slice_up:
        ...
        data.sel(energy_corr=slice(energy_slice_down, energy_slice_up)).sum(
            dim='energy_corr').plot()
